...
export default class extends KauriCommand {
  ...
  public async exec(message: Message, { command }: CommandArgs): Promise<void> {
    ...
    if (!command) {
      const embed = new MessageEmbed()
        ...
        .setDescription(
          stripIndents`Command prefix: \`${prefix}\`
      ...
      for (const [catId, cat] of this.handler.categories) {
        ...
        if (cmds.length !== 0) {
          embed.addFields({ name: `**${title}**`, value: cmds.map(c => `\`${c.aliases[0]}\``).join(', ') });
        }
      }
    ...
    message.util?.send({ embeds: [command.help(message)] });
  }
}
