...
export default class TimerCommand extends KauriCommand {
  ...
  private pauseTimer(message: Message, target: string, duration?: number) {
    ...
    timer.paused = true;
    if (duration) timer.pauseTimeout = this.client.setTimeout(() => (timer.paused = false), duration * 1000, timer);
  ...
  private resumeTimer(message: Message, target: string) {
    ...
    timer.paused = false;
    if (timer.pauseTimeout) this.client.clearTimeout(timer.pauseTimeout);
  ...
  public async exec(message: Message, { operation, duration, target, users }: CommandArgs) {
    switch (operation) {
      ...
      case 'pause':
        if (!duration || !target)
          return this.client.logger.parseError(new Error('Parameters missing from timer#pause'));
      ...
      case 'extend':
        if (!duration || !target)
          return this.client.logger.parseError(new Error('Parameters missing from timer#extend'));
