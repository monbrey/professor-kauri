...
export default class AuctionCommand extends KauriCommand {
  ...
  public async exec(message: Message, args: PokemonCommandArgs | TextCommandArgs) {
    ...
    const w1 = setTimeout(
      () =>
        message.channel.send(`${bid.member ? bid.member.displayName : 'Starting'} at ${bid.value.to$()} - going once!`),
      20000,
      bid,
    );
    const w2 = setTimeout(
      () =>
        message.channel.send(
          `${bid.member ? bid.member.displayName : 'Starting'} at ${bid.value.to$()} - going twice!`,
        ),
      40000,
      bid,
    );
    ...
    collector.on('end', () => {
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(
          stripIndents`${name} sold to ${
            bid.member.displayName
          } for ${bid.value.to$()}. Head over to the [Auction Room](https://forum.pokemonurpg.com/showthread.php?tid=1719) to claim! Include the link to this message:`,
        );
