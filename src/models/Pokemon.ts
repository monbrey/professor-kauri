...
export class Pokemon {
  ...
  public get prices(): string[] | null {
    if (this.pokemart && this.berryStore) {
      return [`Pokemart: ${this.pokemart.to$()}`, `Berry Store: ${this.berryStore.to$()}`];
    }
  ...
  async dex(client: KauriClient, query?: string, rating?: number): Promise<MessageEmbed> {
  ...
  async learnset(query?: string, rating?: number): Promise<MessageEmbed> {
    const color = await Color.getColorForType(this.type1.toLowerCase());
