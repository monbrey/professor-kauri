...
export default class MessageListener extends Listener {
  ...
  public exec(message: Message) {
    ...
    if (message.mentions.users.size > 5) {
      ...
      this.client.logger.major(message.guild!, "Spam protection", `${message.author.tag} (${message.author.id}) banned for mention spam`);
    }
    ...
    if (count > 5) {
      ...
      this.client.logger.major(message.guild!, "Spam protection", `${message.author.tag} (${message.author.id}) banned for message spam`);
    }
  }
}
