...
export default class extends KauriSlashCommand {
  ...
  public async exec(interaction: CommandInteraction, { attacker, defender }: Record<string, string>): Promise<void> {
    if (!attacker) throw new CommandExecutionError("Required command parameter 'attacker' not found");
    if (!defender) throw new CommandExecutionError("Required command parameter 'defender' not found");
  ...
  private calcElectro(attacker: number, defender: number): number {
    ...
    if (percentage.between(33.34, 50)) {
      return 80;
    }
    if (percentage.between(25.01, 33.33)) {
      return 120;
    }
    if (percentage.between(0.01, 25)) {
      return 150;
    }
