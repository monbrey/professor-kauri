...
export class Pokemon {
  ...
  static async fetch(client: KauriClient, name: string): Promise<Pokemon | null> {
    const database = await client.getDatabase();
    const data = await database.collection("pokemon").findOne({ name });
  ...
  public get prices(): string[] | null {
    if (this.pokemart && this.berryStore) {
      return [
        `Pokemart: ${this.pokemart.toLocaleString("en-US", { style: "currency" })}`,
        `Berry Store: ${this.berryStore.toLocaleString("en-US", { style: "currency" })}`,
      ];
    }
    if (this.pokemart) return [`Pokemart: ${this.pokemart.toLocaleString("en-US", { style: "currency" })}`];
    if (this.berryStore) return [`Berry Store: ${this.berryStore.toLocaleString("en-US", { style: "currency" })}`];
    return null;
  }
      ...
      .addFields([
        {
          ...
          value: `${t1} ${titleCase(this.type1)}${this.type2 ? ` | ${titleCase(this.type2)} ${t2}` : ""}`,
        },
        {
          name: "**Abilities**",
